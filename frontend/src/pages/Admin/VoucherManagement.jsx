...
import {
  AlertCircle, Check, Coins, Edit2, Gift, Loader2, Percent,
...
export default function VoucherManagement() {
  ...
  const open = (template = null) => {
    setEditing(template);
    setForm(template
      ? { ...template, serviceId: template.serviceId?._id || template.serviceId || '' }
      : emptyForm);
    setError('');
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-md" onMouseDown={(event) => { if (event.target === event.currentTarget && !saving) setEditing(undefined); }}>
          <form onSubmit={save} className="relative w-full max-w-2xl overflow-hidden rounded-[1.5rem] border border-white/[0.1] bg-[#101111] shadow-[0_30px_100px_rgba(0,0,0,0.55)]">
            ...
            <div className="grid gap-5 p-6 sm:grid-cols-2">
              ...
              <label><span className="text-xs font-bold text-white/55">Reward type</span><select value={form.type} onChange={(event) => setForm({ ...form, type: event.target.value })} className={fieldClass}><option value="PERCENT_DISCOUNT">Percentage discount</option><option value="FREE_SERVICE">Free service</option></select></label>
              <label><span className="text-xs font-bold text-white/55">Point cost</span><div className="relative"><input type="number" min="1" required value={form.pointCost} onChange={(event) => setForm({ ...form, pointCost: event.target.value })} className={`${fieldClass} pr-16 font-mono tabular-nums`} /><span className="pointer-events-none absolute bottom-3 right-3 text-[10px] font-black uppercase tracking-widest text-white/25">points</span></div></label>
              {form.type === 'PERCENT_DISCOUNT' ? (
                <label className="sm:col-span-2"><span className="text-xs font-bold text-white/55">Discount percentage</span><div className="relative"><input type="number" min="1" max="100" required value={form.discountPercent} onChange={(event) => setForm({ ...form, discountPercent: event.target.value })} className={`${fieldClass} pr-12 font-mono tabular-nums`} /><Percent size={15} className="pointer-events-none absolute bottom-3.5 right-3 text-white/25" /></div></label>
              ) : (
                <label className="sm:col-span-2"><span className="text-xs font-bold text-white/55">Included service</span><select required value={form.serviceId} onChange={(event) => setForm({ ...form, serviceId: event.target.value })} className={fieldClass}><option value="">Select an active service</option>{services.map((service) => <option key={service._id} value={service._id}>{service.name} · {Number(service.price || 0).toLocaleString('vi-VN')} VND</option>)}</select></label>
              )}
