...
import { Bell, Check, Send, X, Trash2, AlertTriangle, Paperclip } from 'lucide-react';
import ValoAIMascot from './ValoAIMascot';
import './ValoAICopilot.css';
...
const typeLabel = { MODIFY_PRICING: 'Điều chỉnh bảng giá', CREATE_TICKET_PACKAGE: 'Tạo gói vé mới', UPDATE_TICKET_PACKAGE: 'Cập nhật gói vé' };
function DraftCard({ draft, busy, onDecide }) {
  ...
  return <div className={`valo-ai-draft ${settled ? `is-${settled}` : ''}`}>
    ...
    {draft.current && <details><summary>Giá trị hiện tại</summary><dl>{Object.entries(draft.current).filter(([key]) => key in fieldLabels).map(([key, value]) => <div key={key}><dt>{fieldLabels[key]}</dt><dd>{pretty(value)}</dd></div>)}</dl></details>}
    {draft.payload && Object.keys(draft.payload).length > 0 && <details open><summary>Đề xuất thay đổi</summary><dl>{Object.entries(draft.payload).map(([key, value]) => <div key={key}><dt>{fieldLabels[key] || key}</dt><dd>{pretty(value)}</dd></div>)}</dl></details>}
...
function Message({ row, busy, onDecide }) {
  ...
  return <div className={`valo-ai-message-row ${user ? 'is-user' : 'is-ai'}`}>
    ...
    <div className="valo-ai-message-stack">
      <div className="valo-ai-message-bubble"><p>{row.message}</p>
        {row.evidence?.length > 0 && <details className="valo-ai-evidence"><summary>Xem dữ liệu nguồn</summary>{row.evidence.map((entry, i) => <p key={i}>{entry.source} · {dateTime(entry.timestamp)}</p>)}</details>}
...
export default function ValoAICopilotView({ position, open, setOpen, tab, setTab, messages, busy, noticeBusy, input, setInput, submit, decide, notifications, unreadCount, filter, setFilter, selected, setSelected, preview, openNotice, refresh, dismissNotice, onPointerDown, onPointerMove, onPointerUp, onTriggerClick, bottomRef, suggestions, filters, dragging }) {
    ...
    <aside aria-label="VALO AI" aria-hidden={!open} inert={!open} className={`valo-ai-panel ${open ? 'is-open' : ''}`} style={popupStyle(position)}>
      ...
      {tab === 'chat' ? <>
        ...
        {selected ? <article className="valo-ai-notification-detail"><button type="button" onClick={() => setSelected(null)}>← Danh sách</button><h3>{selected.title}</h3><p>{selected.summary}</p><small>{selected.severity} · {dateTime(selected.detectedAt)}</small><h4>Bằng chứng</h4><pre>{JSON.stringify(selected.evidence, null, 2)}</pre>{selected.recommendedActions?.map((action, i) => <p key={i}>• {action}</p>)}{selected.status === 'RESOLVED' && <p className="resolved"><Check size={15}/> Đã được {selected.resolvedBy?.username || 'Admin'} xử lý lúc {dateTime(selected.resolvedAt)}</p>}{selected.status === 'CLEARED' && <p>Tín hiệu đã trở về mức thông thường.</p>}</article>
          : notifications.length ? notifications.map((item) => <article key={item._id} className="valo-ai-notification"><button type="button" onClick={() => openNotice(item._id)}><small className={item.severity === 'CRITICAL' ? 'critical' : ''}>{item.severity} {!item.isRead && '●'}</small><strong>{item.title}</strong><p>{item.summary}</p>{item.status === 'RESOLVED' && <em>Đã xử lý bởi {item.resolvedBy?.username || 'Admin'}</em>}{item.status === 'CLEARED' && <em>Tín hiệu đã trở về mức thông thường</em>}</button><button type="button" aria-label="Bỏ qua cảnh báo" onClick={() => dismissNotice(item._id)}><Trash2 size={16}/></button></article>) : <p className="valo-ai-empty">Hiện chưa có vấn đề đáng chú ý.</p>}
      </div>}
